...
class MultiAgentEnv(gym.Env):
    ...
    def reset_env(self):
        ...
        for i in range(self.num_User):
            if i >= self.num_User / 2:

                loc = []
                loc_x = np.random.randint(low=0, high=200)
                loc_y = np.random.randint(low=-100, high=100)
                loc.append(loc_x)
                loc.append(loc_y)
                self.User[i].loc = loc
            else:
                loc = []
                loc_x = np.random.randint(low=-200, high=0)
                loc_y = np.random.randint(low=-100, high=100)
                loc.append(loc_x)
                loc.append(loc_y)
                self.User[i].loc = loc
        # initialize the location of ES
        self.Server[0].loc = [-100, 0]
        self.Server[1].loc = [100, 0]
    ...
    def trans_action_for_twin_ddpg(self,action, users_num, server_num ):
        ...
        off_th = 2/(server_num+1)
        for i in range(len(o)):
            if o[i]< (-1 + off_th):
                o_new[i] = 0
            elif o[i] >= (-1+off_th) and o[i] < (-1+2*off_th):
                o_new[i] = 1
            else:
                o_new[i] = 2
        b = action[users_num:2 * users_num]
        f =action[2 * users_num:]
        # print("1offload",o)
        # print("1bandwidth",b)
        # print("1frequency",f)
